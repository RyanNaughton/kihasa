Dchebpoly_deriv <- function(n,x,T,dx1,dx2) {
  ...
  # Set initial space
  dT1 <- matrix(0, nrow = length(x), ncol = n)
  dT2 <- matrix(0, nrow = length(x), ncol = n)
  ddT1<- matrix(0, nrow = length(x), ncol = n)
  ddT2<- matrix(0, nrow = length(x), ncol = n)
  ...
  return <- list('dT1'=dT1,'dT2'=dT2,'ddT1'=ddT1,'ddT2'=ddT2)
}
